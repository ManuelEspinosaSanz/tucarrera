...
import type { CareerResult, SeasonStats } from "./types";
 ...
 * of choice ids they picked. This is what makes `/carrera/[id]` work without a
 * database — the URL encodes the inputs, and the deterministic seed does the rest.
 *
 * Event occurrence (which season, which event) never depends on which choice was
 * picked, only on the rng stream and prior event ids — so replaying the same
 * decisions in the same order always reproduces the same result.
 */
export function replayCareer(options: CareerOptions, decisions: readonly string[]): CareerResult {
  ...
  for (let seasonNumber = 1; seasonNumber <= MAX_SEASONS; seasonNumber++) {
    ...
    if (event) {
      ...
      const choiceId = decisions[decisionIndex];
      decisionIndex++;
